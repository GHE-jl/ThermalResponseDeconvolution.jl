...
One requirement on the input data is a **constant time step**. Here `t` is a time array
...
to its initial value). A second requirement on the input data is that the initial time step (at `t = 0`)
corresponds to the initial ground temperature `T_g`, such that:
...
- `n` is the number of log-spaced nodes the optimization solves for `ĝ`, returned at
  every index `t` and interpolated from those nodes `gOpt(τ)` (see [`set_nodes`](@ref) to recover their
...
Since `ĝ(t)` is a thermal response function, convolving it back with `f` should reproduce the
...
`gOpt(τ)` holds the optimized values *before* interpolation, at the node indices returned by
